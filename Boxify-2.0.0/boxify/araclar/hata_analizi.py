...
QProgressBar {
    ...
    background-color: #333; text-align: center; color: white; height: 18px;
}
...
QTabBar::tab {
    background: #333; color: #bbb; padding: 5px 12px;
    border: 1px solid #3a3a3a; border-bottom: none;
}
QTabBar::tab:selected { background: #0d7acc; color: white; }
...
COL_TP = "#0d7acc"      # doğru tespit        — düz mavi
...
class PreviewCanvas(QWidget):
    ...
    def paintEvent(self, ev):
        ...
        legend = [("düz mavi: doğru", COL_TP), ("noktalı sarı: uydurma", COL_FP),
                  ("kalın kesikli: kaçırılan", COL_FN),
                  ("mor: sınıf karışıklığı", COL_CONF),
                  ("ince kesikli: referans", COL_GT)]
        x = 6
        for text, col in legend:
            p.setPen(QColor(col))
            p.drawText(QRectF(x, self.height() - 17, 240, 15), Qt.AlignVCenter, text)
            x += 8 + 6.4 * len(text)
...
class MainWindow(QMainWindow):
    ...
    def _build_left(self) -> QWidget:
        ...
        self.count_lbl.setStyleSheet("color:#777; font-size:11px;")
    ...
    def _build_right(self) -> QWidget:
        ...
        self.model_info.setStyleSheet("color:#8f8f8f; font-size:11px;")
    ...
    def _build_active_tab(self) -> QWidget:
        ...
        hint.setStyleSheet("color:#8f8f8f; font-size:11px;")
        v.addWidget(hint)
        v.addStretch()
        return w
